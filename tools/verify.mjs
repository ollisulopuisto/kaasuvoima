 ...
 */
{
  ...
  const pp = await page.evaluate(async ({ jobs, todayDay }) => {
    ...
    /* --- 7. neljäs valikkorivi mahtuu ruudulle --- */
    {
      /*
      ...
      const lowest = (hasSave) => {
        const stand = Object.create(game);
        const title = new TitleScene(stand);
        title.options = hasSave
          ? ['JATKA PELIÄ', 'UUSI PELI', d.DAILY_TITLE, 'PARHAAT PIERUT']
          : ['UUSI PELI', d.DAILY_TITLE, 'PARHAAT PIERUT'];
        title.tick = 40;
        title.puffs = [];
      ...
      const four = lowest(true);
      const three = lowest(false);
      expect('alkuruudun valikko ja sen opaste mahtuvat ruudulle',
        four <= 239 && three <= 239, `neljä riviä ${four}, kolme riviä ${three}`);
    }
