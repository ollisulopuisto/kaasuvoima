 ...
 * satunnaisuutta/panoksia ja generoi 2–5 muunlaista bonusasiaa, esim. laita
 * panos ja lyö vetoa jne; enempi tuuria kuin taitoa."*
 *
 * Rajaus on siinä viimeisessä lauseessa ja se on hyvä: **bonushuone jossa taito
 * ratkaisee on vain lyhyt kenttä.** Kenttiä on jo kuusikymmentäviisi. Se mitä
 * kartalla voi tehdä ja mitä kentässä ei, on panos — kolikot ovat pelin ainoa
 * kertyvä valuutta jolla on jo merkitys (sata on elämä), joten niiden
 * asettaminen alttiiksi on ainoa panos jonka pelaaja osaa lukea ilman että
 * mitään pitää selittää.
 *
 * Talot ovat kartan solmuja jotka olivat jo olemassa — jokaisessa maailmassa
 * yksi — eikä yhtään uutta solmua lisätty. Se on tarkoituksellista: kartan
 * muoto on mitattu (jokainen kenttä on tiellä linnakkeeseen, joka haara on
 * kuvattu), ja uusi solmu olisi ollut uusi tie. Talo sai luonteen, ei osoitetta.
 *
 *   `items`     HERNETALO. Kolme tuttua esinettä ja **arpa**: neljäs luukku on
 *               nyt tuntematon, ja se voi olla parempi (tähti) tai tyhjä.
 *               Varma valinta säilyy, joten arpa on tarjous eikä pakko.
 *   `coinflip`  KRUUNA VAI PIERU. Panosta kolikoita, kolikko lentää ilmaan.
 *               Voitto tuplaa, häviö vie. Puhdasta tuuria.
 *   `cups`      KOLME KUPPIA. Yhden alla on elämä, kupit sekoitetaan. Katse
 *               auttaa vähän ja onni loput.
 *   `bet`       VETOTALO. Lyö vetoa **omasta pelaamisestasi**: läpäise
  ...
  drawTerrain(ctx) {
    ...
    for (let ty = 0; ty < this.world.terrain.length; ty++) {
      ...
      for (let tx = from; tx <= last; tx++) {
        ...
        drawScenery(ctx, ch, x, y, { theme: th, tick: this.tick, tx, ty });
      }
    }
    ctx.restore();
  }
